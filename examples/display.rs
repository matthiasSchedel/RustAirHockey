...
fn main() -> ! {
    ...
    loop {
        ...
        draw_circle(&mut layer1, touch.x as usize, touch.y as usize, 20, Color::from_hex(0xff0_000))
...
pub fn draw_circle(
    ...
    for y in y_centre - radius..=y_centre + radius {
        for x in x_centre - radius..=x_centre + radius {
            if x * x + x_centre * x_centre - 2 * x * x_centre + y * y
                + y_centre * y_centre - 2 * y * y_centre
                <= radius * radius
            {
